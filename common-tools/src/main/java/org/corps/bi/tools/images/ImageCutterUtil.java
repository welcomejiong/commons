package org.corps.bi.tools.images;
...
public class ImageCutterUtil {
	...
	public enum IMAGE_FORMAT {
		BMP("bmp"), JPG("jpg"), WBMP("wbmp"), JPEG("jpeg"), PNG("png"), GIF(
				"gif");
	 ...
	 * @throws IOException
	 */
	public static String getImageFormatName(ImageInputStream iis)
			throws IOException {
		String formatName = null;
		Iterator<ImageReader> imageReader = ImageIO.getImageReaders(iis);
		if (imageReader.hasNext()) {
	 ...
	 * @throws IOException
	 */
	public static String zoom(String sourcePath, String formatName,
			String targetPath, int width, int height) throws IOException {
		// GIF需要特殊处理
		if (IMAGE_FORMAT.GIF.getValue() == formatName) {
			GifDecoder decoder = new GifDecoder();
			int status = decoder.read(sourcePath);
	 ...
	 * @throws IOException
	 */
	public static String zoom(InputStream inputStream, String formatName,
			String targetPath, int width, int height) throws IOException {
		// GIF需要特殊处理
		if (IMAGE_FORMAT.GIF.getValue().equalsIgnoreCase(formatName)) {
			GifDecoder decoder = new GifDecoder();
		...
		if (imageReaders.hasNext()) {
			...
			for (int i = 0; i < imageNumber; i++) {
				...
				if (sourceImage.getWidth() > image.getWidth()
					...
					image = zoom(image, sourceImage.getWidth(),
